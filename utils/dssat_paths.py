...
def find_dssatpro_file() -> str:
    """Find DSSATPRO file location based on operating system."""
    try:
        ...
        if dssat_env:
            env_path = os.path.join(dssat_env, file_name)
        ...
        if is_windows:
            ...
            applications_path = f'/Applications/DSSAT48/{file_name}'
            if os.path.exists(applications_path):
                return applications_path

            # Fall back to other common macOS installation locations
            possible_paths = [
        ...
        # Search all paths for DSSATPRO file
...
def get_dssat_base() -> str:
    """Get DSSAT base directory from DSSATPRO file"""
    try:
        is_windows = platform.system() == 'Windows'
        v48_path = find_dssatpro_file()
        
        # For macOS, if we found the file directly in Applications, return the base path
        if not is_windows and v48_path == '/Applications/DSSAT48/DSSATPRO.L48':
            ...
            for line in file:
                if line.strip().startswith('DDB'):
                    ...
                    if len(parts) >= 2:
                        ...
                        dssat_path = dssat_path.replace('\\', '/' if not is_windows else '\\')
                        
                        # For macOS, handle potential Windows paths in the config file
                        if not is_windows and ':' in dssat_path:
        ...
        # If we couldn't find it in the file, return the known path for macOS
        if not is_windows:
            return '/Applications/DSSAT48'
        ...
        # Fallback to known location for macOS
        if not is_windows:
            return '/Applications/DSSAT48'
        raise
...
def get_crop_details() -> List[dict]:
    ...
    try:
        ...
        dssatpro_path = os.path.join(DSSAT_BASE, 'DSSATPRO.V48' if is_windows else 'DSSATPRO.L48')
        crop_details = []
                ...
                if len(parts) >= 2:
                    ...
                    if folder_code.endswith('D'):
                        ...
                        if is_windows:
                            ...
                            directory = directory.replace('\\', '/')
                            # Ensure the path starts with /Applications/DSSAT48
                            if not directory.startswith('/Applications/DSSAT48'):
                                directory = os.path.join('/Applications/DSSAT48', os.path.basename(directory))
...
def initialize_dssat_paths():
    ...
    try:
        ...
        if not is_windows:
            ...
            dssat_exe = os.path.join(dssat_base, "DSCSM048")
        else:
            dssat_base = get_dssat_base()
            print(f"DSSAT Base Directory: {dssat_base}")
